        ...
        <form method="get" action="{{ route('tenant.admin.audit-log.index') }}" class="mb-6 flex flex-wrap items-end gap-3">
            <div class="form-control min-w-[200px]">
                <label for="user_id" class="label py-1">
                    <span class="label-text text-xs">Utente staff</span>
                </label>
            ...
            @if ($filterUserId !== '')
                <a href="{{ route('tenant.admin.audit-log.index') }}" class="btn btn-ghost">Reset</a>
            @endif
        </form>
                                ...
                                <td class="text-xs text-base-content/70">
                                    @if ($log->metadata)
                                        <details class="cursor-pointer">
                                            ...
                                            <pre class="mt-2 max-h-40 overflow-auto rounded-lg bg-base-300 p-2 text-[11px]">{{ json_encode($log->metadata, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                                        </details>
                                    @else
                                        —
                                    @endif
