...
<x-layouts.tenant :title="$course->title.' — Dettaglio tempi'">
    <x-ui.page>
        <x-ui.page-header title="Dettaglio tempi">
            <x-slot:breadcrumb>
                <a href="{{ route('tenant.admin.courses.learners', $course) }}" class="link link-hover">Corsisti</a>
                <span aria-hidden="true">/</span>
                <span class="text-base-content/80">Dettaglio tempi</span>
            </x-slot:breadcrumb>
            <x-slot:subtitle>
                <span class="font-medium text-base-content">{{ $enrollment->user?->name ?? '—' }}</span>
        ...
        @if (($sessionSummary['count'] ?? 0) > 0)
            <div class="grid gap-3 border-b border-base-300 px-4 py-5 sm:grid-cols-2 lg:grid-cols-4 lg:px-6">
                <x-ui.stat
                    title="Sessioni"
        ...
        <div class="border-b border-base-300 px-4 py-4 lg:px-6">
            ...
            <p class="mt-1 text-xs text-base-content/60">
                Ogni riga è un segmento di visione continua (video o SCORM); Inizio e Fine coincidono con la riproduzione attiva
                @tenantcan('content.courses.manage')
                    · modifica inline disponibile
                @endtenantcan
                .
            </p>
        </div>
        ...
        <div class="overflow-x-auto border-b border-base-300">
            <table class="table table-zebra w-full min-w-[640px]">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="relative text-right">
                                <details class="group/edit inline-block text-left">
                                    ...
                                    <div class="absolute end-0 top-full z-50 mt-2 w-[min(calc(100vw-2rem),22rem)] border border-base-300 bg-base-100 p-4">
                                        <form class="space-y-3" method="post" action="{{ route('tenant.admin.courses.learners.time.sessions.update', [$course, $enrollment, $s->id]) }}">
                                            ...
                                            <div class="form-control w-full">
                                                <label class="label py-1" for="started_at_{{ $s->id }}">
                                                    <span class="label-text text-[11px] font-semibold uppercase tracking-wider text-base-content/70">Inizio</span>
                                                </label>
                                            ...
                                            <div class="form-control w-full">
                                                <label class="label py-1" for="ended_at_{{ $s->id }}">
                                                    <span class="label-text text-[11px] font-semibold uppercase tracking-wider text-base-content/70">Fine</span>
                                                </label>
                                            ...
                                            <div class="grid grid-cols-2 gap-3">
                                                <div class="form-control w-full">
                                                    <label class="label py-1" for="video_minutes_{{ $s->id }}">
                                                        <span class="label-text text-[11px] font-semibold uppercase tracking-wider text-base-content/70">Video (min)</span>
                                                    </label>
                                                ...
                                                <div class="form-control w-full">
                                                    <label class="label py-1" for="scorm_minutes_{{ $s->id }}">
                                                        <span class="label-text text-[11px] font-semibold uppercase tracking-wider text-base-content/70">SCORM (min)</span>
                                                    </label>
                    ...
                    @empty
                        <tr>
                            <td class="py-12 text-center" colspan="{{ \App\Support\TenantPermissions::allows(auth()->user(), 'content.courses.manage') ? 9 : 8 }}">
