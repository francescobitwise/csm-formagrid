<x-layouts.tenant :title="'Import allievi CSV — '.tenant('id')">
    <div class="mx-auto max-w-2xl px-6 py-10">
        <x-ui.page>
            ...
            <div class="mt-4">
                <x-ui.page-header title="Import da CSV" subtitle="File UTF-8 con intestazione sulla prima riga." />
            </div>

            <div class="card bg-base-100 shadow-xl mt-6">
                <div class="card-body space-y-4 text-sm text-base-content/70">
                    <p class="font-medium">Colonne supportate</p>
                    ...
                    <p class="rounded-lg border border-base-300 bg-base-200/50 p-3 font-mono text-xs">
                    ...
                    <p class="text-xs text-base-content/60">Le email già presenti in questa organizzazione vengono saltate (riga segnalata nel riepilogo).</p>
                </div>
            </div>

            <div class="card bg-base-100 shadow-xl mt-6">
                <div class="card-body">
                    <form method="post" action="{{ route('tenant.admin.learners.import.store') }}" enctype="multipart/form-data" class="space-y-5">
                        @csrf
                        ...
                        <label class="flex items-center gap-2 text-sm">
                            ...
                            <input type="checkbox" name="send_credentials_email" value="1" class="checkbox checkbox-primary" checked>
                            Invia email con credenziali a ogni nuovo allievo importato
                        </label>

                        <div class="flex gap-3 pt-2">
                            <button type="submit" class="btn btn-primary">Avvia import</button>
